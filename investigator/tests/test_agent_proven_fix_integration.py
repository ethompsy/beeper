...
class TestProvenFixPipelineIntegration:
    def test_proven_fix_is_step_13(self):
        ...
        assert len(steps) == 16
        assert isinstance(steps[15], ProvenFixAccumulatorStep)
        assert steps[15].name == "Proven Fix Accumulation"
    ...
    def test_step_always_included(self):
        ...
        assert isinstance(steps_tl1[15], ProvenFixAccumulatorStep)
        assert len(steps_tl5) == 16
        assert isinstance(steps_tl5[15], ProvenFixAccumulatorStep)
    ...
    def test_step_after_trust_gate(self):
        ...
        assert isinstance(steps[14], TrustGateStep)
        assert isinstance(steps[15], ProvenFixAccumulatorStep)
