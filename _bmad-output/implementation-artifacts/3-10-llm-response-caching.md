...
- [ ] Task 1: Create LlmResponseCache class in `llm/cache.py` (AC: 1, 2)
  - [ ] 1.1 Create `investigator/beeper_investigator/llm/cache.py` with `LlmResponseCache` class
  - [ ] 1.2 Add `_generate_cache_key(messages, model, max_tokens, temperature) -> str` static method using `hashlib.sha256` on `json.dumps(messages, sort_keys=True)` + model + str(max_tokens) + str(temperature)
  - [ ] 1.3 Add `_entries: dict[str, CacheEntry]` instance variable storing `CacheEntry(response: str, created_at: float, model: str)`
  - [ ] 1.4 Add `get(messages, model, max_tokens, temperature) -> str | None` method — returns cached response if key exists and TTL not expired, else None
  - [ ] 1.5 Add `put(messages, model, max_tokens, temperature, response) -> None` method — stores response with current timestamp; evicts oldest entry if `_max_entries` exceeded
  - [ ] 1.6 Add `clear() -> None` method to invalidate all cache entries
  - [ ] 1.7 Add `CacheEntry` dataclass: `response: str`, `created_at: float`, `model: str`

- [ ] Task 2: Add cache configuration to LlmConfig (AC: 3)
  - [ ] 2.1 Add `cache_ttl_seconds: int = 3600` field to `LlmConfig` dataclass
  - [ ] 2.2 Add `cache_max_entries: int = 256` field to `LlmConfig` dataclass
  - [ ] 2.3 Add `cache_enabled: bool = True` field to `LlmConfig` dataclass
  - [ ] 2.4 Load from env vars in `from_env()`: `BEEPER_LLM_CACHE_TTL_SECONDS`, `BEEPER_LLM_CACHE_MAX_ENTRIES`, `BEEPER_LLM_CACHE_ENABLED`

- [ ] Task 3: Integrate cache into LlmClient (AC: 1, 2, 3)
  - [ ] 3.1 Add `_cache: LlmResponseCache` instance variable in `LlmClient.__init__()`, initialized with config TTL and max_entries
  - [ ] 3.2 In `complete_sync()`: before calling `litellm.completion()`, check `self._cache.get()` — if hit, log at INFO and return cached response; still increment `_model_usage` with a `(cached)` suffix
  - [ ] 3.3 In `complete_sync()`: after successful `litellm.completion()`, call `self._cache.put()` to store result
  - [ ] 3.4 In `complete()` (async): same cache check/store pattern as `complete_sync()`
  - [ ] 3.5 Add `clear_cache() -> None` proxy method on `LlmClient`
  - [ ] 3.6 Skip caching when `self.config.cache_enabled is False`

- [ ] Task 4: Add cache metrics tracking (AC: 4)
  - [ ] 4.1 Add `_cache_hits: int` and `_cache_misses: int` counters to `LlmResponseCache`
  - [ ] 4.2 Increment `_cache_hits` in `get()` on hit, `_cache_misses` on miss
  - [ ] 4.3 Add `get_cache_stats() -> dict[str, Any]` method returning: `hits`, `misses`, `hit_rate` (float 0-1), `entries_count`, `max_entries`, `ttl_seconds`
  - [ ] 4.4 Add `reset_stats() -> None` method

- [ ] Task 5: Propagate cache stats to InvestigationResult (AC: 4)
  - [ ] 5.1 In `agent.py`, after `_run_steps()` completes, call `self.llm_client.get_cache_stats()` and include in `InvestigationResult.metadata` under key `cache_stats`
  - [ ] 5.2 Verify `cache_stats` is NOT in `_RESERVED_KEYS` set (it is not)

- [ ] Task 6: Tests for LlmResponseCache (AC: all)
  - [ ] 6.1 Test `_generate_cache_key()` produces deterministic hash for same inputs
  - [ ] 6.2 Test `_generate_cache_key()` produces different hash for different messages
  - [ ] 6.3 Test `_generate_cache_key()` produces different hash for different model/max_tokens/temperature
  - [ ] 6.4 Test `get()` returns None on cache miss
  - [ ] 6.5 Test `put()` then `get()` returns cached response (cache hit)
  - [ ] 6.6 Test `get()` returns None after TTL expires (use time mocking)
  - [ ] 6.7 Test `put()` evicts oldest entry when `_max_entries` exceeded
  - [ ] 6.8 Test `clear()` removes all entries
  - [ ] 6.9 Test cache hit/miss counters are incremented correctly
  - [ ] 6.10 Test `get_cache_stats()` returns correct hit_rate calculation
  - [ ] 6.11 Test `reset_stats()` clears counters

- [ ] Task 7: Tests for LlmClient cache integration (AC: 1, 2, 3)
  - [ ] 7.1 Test `complete_sync()` returns cached response on cache hit (litellm.completion NOT called)
  - [ ] 7.2 Test `complete_sync()` calls litellm.completion on cache miss and stores result
  - [ ] 7.3 Test `complete_sync()` with different models produces different cache keys (no cross-model pollution)
  - [ ] 7.4 Test `complete()` async returns cached response on hit
  - [ ] 7.5 Test `clear_cache()` invalidates all cached responses
  - [ ] 7.6 Test cache disabled via `cache_enabled=False` — always calls litellm
  - [ ] 7.7 Test existing LLM mocks still work (backward compatibility)
  - [ ] 7.8 Test `_model_usage` tracks cached calls with `(cached)` suffix

- [ ] Task 8: Tests for cache stats propagation (AC: 4)
  - [ ] 8.1 Test `cache_stats` propagated to `InvestigationResult.metadata`
  - [ ] 8.2 Test `cache_stats` contains expected keys (hits, misses, hit_rate, entries_count)

- [ ] Task 9: LlmConfig cache configuration tests (AC: 3)
  - [ ] 9.1 Test `LlmConfig` cache fields have correct defaults (TTL=3600, max=256, enabled=True)
  - [ ] 9.2 Test `from_env()` loads cache env vars
  - [ ] 9.3 Test `from_env()` handles missing cache env vars (uses defaults)
