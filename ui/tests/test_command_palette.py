...
@pytest.fixture
def palette_html(app: Flask) -> str:
    """Render command palette template."""
...
@pytest.fixture
def help_html(app: Flask) -> str:
    """Render keyboard help template."""
    with app.app_context():
        template = app.jinja_env.get_template(
...
def _read_static(relative_path: str) -> str:
    """Read a static file relative to the static directory."""
...
class TestCommandPaletteTemplate:
    ...
    def test_command_palette_has_aria_combobox(self, palette_html: str) -> None:
        """Command palette search input should have role=combobox."""
        assert 'role="combobox"' in palette_html

    def test_command_palette_has_aria_owns(self, palette_html: str) -> None:
        """Search input should reference results list via aria-owns."""
        assert 'aria-owns="command-results"' in palette_html

    def test_command_palette_has_aria_autocomplete(self, palette_html: str) -> None:
        """Search input should have aria-autocomplete=list."""
        assert 'aria-autocomplete="list"' in palette_html

    def test_command_palette_has_aria_label(self, palette_html: str) -> None:
        """Search input should have a descriptive aria-label."""
        assert 'aria-label="Search commands and navigation"' in palette_html

    def test_command_palette_has_aria_expanded(self, palette_html: str) -> None:
        """Search input should start with aria-expanded=false."""
        assert 'aria-expanded="false"' in palette_html

    def test_command_palette_has_results_listbox(self, palette_html: str) -> None:
        """Results list should have role=listbox."""
        assert 'role="listbox"' in palette_html

    def test_command_palette_overlay_hidden_by_default(
        self, palette_html: str
    ) -> None:
        """Overlay should not have 'active' class by default."""
        assert "command-palette-overlay" in palette_html
        assert "active" not in palette_html.split("command-palette-overlay")[1].split('"')[0]


class TestKeyboardHelpTemplate:
    """Test keyboard help overlay template."""

    def test_keyboard_help_contains_cmd_k_shortcut(self, help_html: str) -> None:
        """Help overlay should show Cmd+K shortcut."""
        assert "Cmd" in help_html
        assert "Open command palette" in help_html

    def test_keyboard_help_contains_navigation_shortcuts(
        self, help_html: str
    ) -> None:
        """Help overlay should list navigation chord shortcuts."""
        assert "Go to Investigations" in help_html
        assert "Go to SLO Dashboard" in help_html
        assert "Go to Knowledge Base" in help_html
        assert "Go to Notifications" in help_html
        assert "Go to Topology" in help_html
        assert "Go to Health" in help_html
        assert "Go to Metrics" in help_html

    def test_keyboard_help_contains_investigation_shortcuts(
        self, help_html: str
    ) -> None:
        """Help overlay should reference investigation actions."""
        assert "Annotate" in help_html
        assert "Redirect" in help_html
        assert "Approve" in help_html
        assert "Reject" in help_html

    def test_keyboard_help_contains_help_shortcut(self, help_html: str) -> None:
        """Help overlay should show the ? shortcut."""
        assert "Toggle this help" in help_html

    def test_keyboard_help_has_close_button(self, help_html: str) -> None:
        """Help overlay should have a close button with no inline JS."""
        assert "keyboard-help-close" in help_html
        assert 'aria-label="Close keyboard shortcuts help"' in help_html
        assert "onclick" not in help_html

    def test_keyboard_help_overlay_hidden_by_default(
        self, help_html: str
    ) -> None:
        """Help overlay should not have 'active' class by default."""
        assert "keyboard-help-overlay" in help_html
        assert "active" not in help_html.split("keyboard-help-overlay")[1].split('"')[0]
...
class TestCommandPaletteStaticFiles:
    ...
    def test_command_palette_js_exists(self) -> None:
        ...
        js_path = os.path.join(STATIC_DIR, "js", "command-palette.js")
...
class TestCommandPaletteCSSIntegration:
    ...
    def test_main_css_has_command_palette_styles(self) -> None:
        ...
        content = _read_static("css/main.css")
