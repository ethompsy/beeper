...
class InvestigationService:
    ...
    # NOTE (Task 6.2a / ADR 0001 §0(b)): `annotate_investigation`,
    # `redirect_investigation`, `approve_fix`, and `reject_fix` were removed
    # from here. Their only callers were the now-deleted SocketIO handlers
    # (`beeper_ui/websocket/investigation.py`), and each POSTed to an
    # operator endpoint (`/annotate`, `/redirect`, `/approve`, `/reject-fix`)
    # that does not exist in `operator/src/api.rs` — the calls always 404'd
    # and were silently swallowed. See ADR §2 for the full inventory.
