...
| Q4 | Local `ollama/qwen3:8b` returns unparseable LLM output (6/6 investigators: "Failed to parse LLM response; using defaults") + is slow (~1–2.5 min/step) | RCA falls back to defaults → 6.3 `[H]` can't produce evidence-backed payment RCA; blocks 3/3 | Open — found 2026-05-31 live. Recommend a higher-quality/faster model or Anthropic for the acceptance demo; or make parsing tolerant of qwen output |
| Q5 | Detection over-sensitivity: fires on system/`unknown`-service metrics (e.g. `system_cpu_time_seconds_total`), creating a baseline investigation backlog even on a fresh cluster | Buries an injected fault; backlog + concurrency=2 makes 3/3 impractically slow; noisy demo | Open — found 2026-05-31. Needs detection tuning / service-attribution filter (cf. Task 3-0e flood fix) |
| Q6 | Investigator SA `beeper-investigator` cannot `list investigations.beeper.dev` (403 in topology step; confirmed via `kubectl auth can-i`) | Topology step degraded; recurring 403s | Open — found 2026-05-31. Add the RBAC rule to the investigator Role (cf. Task 3-0h investigator RBAC) |
