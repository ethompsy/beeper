...
**Expansion added 2026-08-11 — Phases 7 and 8 below.** The original plan stays complete and untouched; Phase 7 (operator reliability, closing Q9/Q11 via [RFC 0003](../specs/rfcs/0003-bounded-investigator-concurrency.md)) and Phase 8 (cross-service correlation, RFC 0001 Phase 3 via [RFC 0005](../specs/rfcs/0005-cross-service-incident-correlation.md), **gated on a go/no-go measurement**) are net-new forward-looking work. See **Execution order** at the head of Phase 7.
...
| Q9 | Phase 1 bounds duplicates *per service* but not *global* concurrent investigations. A wide outage (or the Q10/Q11 warmup burst) → ~#services concurrent investigator pods → single-node + local-LLM RAM starvation (live: ~92 MB free, 0/23 complete). | Refines Q-E (which dropped the "blunt cap"). For single-node/local-LLM a global limit IS needed — but as a **work-queue that defers, not drops**. | **DESIGNED → [RFC 0003](../specs/rfcs/0003-bounded-investigator-concurrency.md)** (2026-08-11). Admission gate in the Investigation controller: `Pending` CRDs *are* the queue (defer, never drop, per FR11); slot occupancy derived from live investigator Jobs, not CRD phase; admission order is a pure, unit-testable total order banded on the operator-configured `impact_score` — deliberately **not** on the attacker-settable `severity`, since ingestion is unauthenticated and two forged >6σ series would otherwise hold both slots forever (RFC 0003 §8.1). Code default `0` = unlimited, chart default `2`, chart flip `[H]`-gated. Also corrects `docs/deployment-guide.md`'s documented default (`5`) which matches neither values file (`2`). Implemented as **Phase 7, Milestone 7.1** below (Tasks 7.2/7.3/7.6). |
...
| Q11 | EWMA detector state is in-memory (rebuilt in `DetectionConsumer::run()`), so every operator restart wipes baselines → full re-warmup → a Q10 false-positive burst. Restarts are routine (rolling updates, crashes). | Each operator restart re-triggers the flood. | **DESIGNED → [RFC 0003](../specs/rfcs/0003-bounded-investigator-concurrency.md) Part B** (2026-08-11). Q14's durable debounce already closed the **creation-path** half; the residual is the ~10-min post-restart *blindness* window. **Adopted:** a startup grace period (default on, 600 s), driven by detector-internal sample/variance state — **not** by `ewma_warmup_samples`, which is known to read 0 while detectors fire. **Deliberately deferred:** durable EWMA snapshots to Qdrant — its marginal value shrank post-Q14 to "avoid a 10-min blind window," while it makes ingestion-path baseline poisoning *permanent* (today every restart expires a poisoned baseline; persistence removes that expiry). RFC 0003 §6 lists the six conditions for promoting it. Implemented as **Phase 7, Milestone 7.2** below (Task 7.5). |
| Q12 | **The EWMA detector runs on RAW cumulative metrics** — Prometheus counters (`*_total`) and histogram buckets (`*_bucket`) are monotonic/cumulative and reset to 0 on pod restart. EWMA-ing raw cumulative values makes normal load ramps look like spikes and counter resets look like drops. | **The actual dominant driver of the warmup "flood,"** correcting the Q10 hypothesis. Live (post-Q10): 23→**21** baseline investigations, ~all on `*_bucket`/`*_total` with genuine >4σ deviations (e.g. `shipping http_server_duration drop to 0` = a restart counter reset). Q10 (zero-variance floor) only removed ~2 — it was NOT the main cause. | **FIX LANDED — PR #20 (open), live-measured 21→11; 0 remaining `_bucket` investigations (skip confirmed).** Detect on **rates/deltas** for `*_total`/`*_count`/`*_sum` (per-second increase) and skip `*_bucket` (mis-keyed without `le`); counter drop-to-lower = reset, not anomaly. `operator/src/detection/metrics.rs`; 4 new tests. Found 2026-06-04 (corrects Q10). |
| Q13 | After Q12 the live baseline is **~11** warmup false-positives, in two residual classes: **(a)** runtime/infra gauges detected on raw value (`otelcol_*`, `jvm_memory_limit/init`, …); **(b)** near-zero/idle **counter-rate** streams (`redis_keyspace_misses_total`, `nginx_connections_handled_total`) where Q10's *relative* floor (5%·~0 ≈ 0) lets tiny movements fire. | The last layer of the noise onion (38k→~11, ~99.97%). **Key finding:** a single global absolute floor **cannot** finish it — idle count-rate noise (0.2→3 events/s) and small-but-real **GC-duration** signal (0.005→0.2 **seconds**/s) overlap in absolute magnitude but live in different units, so any one scalar either lets noise through or **blinds GC** (a real cross-service incident symptom). | **Q13a/Q13b LANDED — PR #21 (open, stacked on #20).** Q13a: denylist `otelcol_`/`jvm_memory_limit`/`jvm_memory_init` (infra/static) — GC and dynamic heap occupancy deliberately KEPT. Q13b: opt-in absolute stddev floor (`BEEPER_DETECTION_ABS_STDDEV_FLOOR`, default 0=off); GC-safety unit-tested. 3 new tests; CI clippy clean. NOT yet live-re-measured (floor off by default → only denylist changes behavior). **Q13c DESIGNED → RFC 0002** (unit-free temporal-persistence dwell, primary; optional unit-aware magnitude gate; gated on a Phase-0 live characterization of transient-vs-sustained noise). Found/designed 2026-06-05. **Composition contract (2026-08-11):** RFC 0002's dwell gate and [RFC 0003](../specs/rfcs/0003-bounded-investigator-concurrency.md) §5.7's startup grace period are both additive gates on *firing* in the same files (`detection/ewma.rs`, `detection/metrics.rs`), so they compose — but whoever lands second must add a `[T]` proving it (a dwell breach inside the grace window still suppresses; after grace, dwell behaves unchanged). Task 7.5 carries this if it goes second. **Q13a/b/RFC0002 MERGED (PRs #20/#21/#22).** **Phase-0 live (2026-06-05):** denylist confirmed working (no `otelcol_`/`jvm_memory_limit`/`system_`/`process_` fired); warmup baseline ≈ **14** (≈1/service) — *not* lower, because the denylisted classes were replaced by the two residual classes (kept GC/heap gauges + near-zero counter-rates), exactly as RFC 0002 predicted. Q13c implementation still pending (now lower priority than Q14). |
| Q14 | **Investigations FAIL on local qwen, then RE-FIRE per service** — the per-service guard (Phase 1) only blocked *non-terminal* investigations, and the in-memory cooldown is anchored at creation + wiped on restart (Q11). So: investigation fails (~30 min in, at the final RCA step) → terminal → guard releases → cooldown long-expired → service re-fires ~2 min later. | **The dominant live baseline polluter on qwen — NOT metric noise.** Live (2026-06-05): accumulated **14 → 104** investigations over 2.5 h (≈6/service every ~30 min), **85 failed** (all at "Generating resolution recommendations"). The recurring "one issue" resurfacing via the *failure* path. | **FIX LANDED + LIVE-VERIFIED — branch `fix/investigation-reopen-debounce`.** Durable, API-derived **post-terminal re-open debounce** in the guard: skip if a terminal investigation's `completed_at` is within a window — longer for Failed (default 1 h) than Completed (30 min). Anchor is API state → survives restart (**closes Q11 for the creation path**). Live: with the debounce on, the detector fired anomalies but the guard suppressed **all** of them → count held flat at 104, **0 created**, guard-skips climbing (`reason="recent failed investigation"`). 7 new tests; 65 detection tests pass; CI clippy clean. **Follow-up:** exponential backoff / per-service retry cap (this slows but doesn't *stop* re-fire of a persistently-failing signal); and **Q4 qwen reliability** (why investigations fail at the final step). Found+fixed 2026-06-05. |
...
**Live-validation pass 4 (2026-06-05) — the central `[H]` blocker is RESOLVED: investigations now COMPLETE on local qwen.** This session landed the full detection-quality + investigator chain that passes 1–3 said was needed: per-service-incident guard (RFC 0001 Phase 1, #17), Q10 zero-variance floor (#19), **Q12 rate-based cumulative detection (#20)**, **Q13a/b infra denylist + opt-in floor (#21)**, RFC 0002 Q13c design (#22), **Q14 durable post-terminal re-open debounce (#23)**, and **Q4 selective qwen3 thinking-disable + deadline headroom (#24)**.
- **Baseline:** 38k → **~14** warmup false-positives (Q5/Q7/Q10/Q12/Q13a, live-measured); **Q14 stops the fail→re-fire accumulation** (was 14→104 over 2.5 h via failed-investigation re-fire; with the debounce the count holds flat — live-verified: 0 created, all anomalies guard-suppressed).
- **Q4 — investigations COMPLETE on local qwen (the "0/47 completed" blocker, gone).** Root cause was the 30-min Job `activeDeadlineSeconds` (every failure ran *exactly* 1800 s): qwen3 generated huge `<think>` blocks at ~15 tok/s → calls hit litellm's 600 s timeout → retried → pipeline overran the deadline → killed mid-final-step. Fix: `/no_think` on the extractive/synthesis steps (impact assessment **12 min → 2 min** live), **kept ON for the deep-RCA hypothesis** (quality where it matters), + per-call timeout + demo deadline 5400 s. **Live 2026-06-05: 2 operator-driven investigations COMPLETED on local qwen** — `image-provider` ~24 min, `beeper` ~19.6 min (both *under* the old 30-min wall), with parseable RCA (the earlier response_parser fix holds).
- **Remaining gate for the `[H]` 3/3 (now purely operational, not code):** (1) merge #23 (Q14) + #24 (Q4) and redeploy so the running cluster has all fixes; (2) choose the provider for the witnessed run — **local qwen now *completes* but is slow** (~20–24 min/investigation → a 3/3 is ~1.5–3 h), whereas **Anthropic** is fast (minutes) *and* strong (the long-standing pass-1–3 recommendation), with the fix being provider-agnostic; (3) run + witness 3 consecutive fault→detect→investigate→complete→recover cycles. Open follow-ups that would further polish but do NOT block: Q13c (residual idle-counter noise, designed in RFC 0002), Q9/Q11 (global work-queue / EWMA persistence), and an operator-controller gap where Jobs GC'd out from under a `running` Investigation leave it orphaned-`running`.
...
## Phase 7: Operator Reliability — Bounded Concurrency & Restart Resilience (Expansion, 2026-08-11)

Closes Open Questions **Q9** and **Q11**. Design: [RFC 0003](../specs/rfcs/0003-bounded-investigator-concurrency.md). Traces to **FR11**, **FR13**, **FR63**, **FR64**, **NFR26**, **NFR27**, **NFR30**, **NFR31**.

### Execution order (whole 2026-08-11 expansion — read this first)

Four scoped candidates were reviewed together (sequencing, feasibility, security). The agreed order across both plan files:

| Wave | Work | Why here |
|---|---|---|
| **0** | **Milestone 7.0 — shared prerequisites** (this file) | Three of these are load-bearing for *multiple* downstream tasks and would otherwise each be re-discovered mid-task as a schedule surprise on the one component this repo cannot integration-test: the chart CRD is hand-written and **silently prunes** unlisted status fields; the detection consumer has no timer and no shutdown channel; the `/api/v1/*` surface has no mechanical authorization guard while **three** concurrent streams are about to add routes; and the operator API is unauthenticated with no NetworkPolicy while two RFCs add endpoints to it |
| **1** | **7.1–7.2** (operator, Rust) · **react-ui 9.1–9.2** (evidence endpoint + React) · **8.1a** (graph-density measurement) · **react-ui 10.0** (Jinja scaffolding + §7 amendment) | Four parallel streams with **zero file overlap** (verified). Operator reliability is position 1 on merit: it is the only candidate whose failure mode is *total* (0/23 and 0/48 completing; 78–92 MB free host RAM), it is triggered by a genuine wide outage — the product's highest-value moment — it has no hard prerequisites, and it is the precondition for evidence checkpointing, the safety net for correlation's live validation, and the thing that makes the demo's 85/104 failure baseline improvable (which determines whether the evidence work has anything to show) |
| **2** | **7.6** (burst soak) · **react-ui 9.3** (structured signals) · **react-ui 10.1–10.2** (Jinja batches A/B) · **8.1b** (pair labelling) | 8.1b wants the calmer baseline 7.6 proves; labelling pairs drawn from a warmup burst measures the wrong world |
| **3** | **8.2 → 8.3** *(only if 8.1a returned go)* · **react-ui 10.3** (Trust/Notifications) · **react-ui 9.4** (checkpointing) | 9.4's default-on flip is hard-gated on 7.6 having proved the cap |
| **4** | **8.4** (= react-ui Task 7.1) **→ 8.5** · **react-ui 10.4** (Jinja cutover) | |
| **Deferred** | RFC 0003 Part B2 (durable EWMA snapshot); react-ui Milestone 2.5 **Phase B** (the 146 `render_template` calls in `knowledge.py`/`investigations.py`) | Phase B is hard-gated behind RFC 0004's evidence work — it deletes the only surface for data with no JSON equivalent yet |

**Requirement/RFC numbers are pre-allocated** so three concurrent streams cannot collide: RFC 0003 owns FR63/FR64/NFR27; RFC 0004 owns FR65/NFR28; RFC 0005 owns NFR29; NFR30/NFR31 are shared and land in Milestone 7.0.

### Milestone 7.0: Shared prerequisites (blocks 7.1+, Phase 8, and react-ui Milestones 2.4/2.5)

| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 7.0a | **CRD schema parity guard** — a test reflecting `InvestigationSpec`/`InvestigationStatus` field names against the hand-written `helm/beeper/templates/crds/investigation-crd.yaml`, both directions | S | None | pending |
| 7.0b | **Mechanical `/api/v1/*` authorization guard** — enumerate `app.url_map`, assert every route declares a role against a committed allowlist (NFR30) | S | None | pending |
| 7.0c | **Detection-consumer select loop + shutdown plumbing** — convert `DetectionConsumer::run()` to `tokio::select!` over `buffer.recv()`, a `tokio::time::interval`, and `shutdown_rx.changed()` | S | None | pending |
| 7.0d | **Operator NetworkPolicy** — ship `helm/beeper/templates/operator-networkpolicy.yaml` restricting :8080 to the UI pod and :9090 to the collector (NFR31) | S | None | pending |

**Task 7.0a AC:** `[T]` every `#[serde]` field on `InvestigationSpec`/`InvestigationStatus` appears in the committed CRD YAML `properties`, and every YAML property maps to a Rust field · `[T]` the guard **fails** when a field is added on one side only (verified by a deliberate one-sided edit, not asserted by construction) · `[T]` `cargo test` green.
*Why this exists:* `grep -rn "crdgen\|CustomResourceExt" operator/src` returns nothing — the chart CRD is hand-written and structural, so an unlisted status field is **silently pruned by the API server** on every write. Both RFC 0003 and RFC 0005 add status fields; RFC 0005 adds ~60 lines of nested schema.

**Task 7.0b AC:** `[T]` for every `app.url_map` rule under `/api/v1/` with a `POST|PUT|PATCH|DELETE` method, the view function declares `required_role` matching a committed allowlist row; every `GET` rule declares one at all · `[T]` adding a route without an allowlist row fails the test · `[T]` the existing `ui/tests/test_json_api_rbac.py` suite passes unmodified alongside it.
*Why this exists:* `require_role("user")` is a literal pass-through (`permissions.py:296–345` branches only on `role == "admin"`), and the repo already demonstrates the omission — `POST /investigations/<id>/{confirm,reject,resolve}` carry no `require_role` while the sibling `/verify` does. Three concurrent streams are about to add `/api/v1/*` routes; this converts "remember the decorator" into a CI failure.

**Task 7.0c AC:** `[T]` `run()` returns on `shutdown_rx.changed()` · `[T]` the periodic tick fires on a wall-clock interval with an **empty** sample buffer (proving it is time-driven, not data-rate-driven) · `[T]` existing detection tests pass unmodified · `[T]` `cargo fmt`/`clippy -D warnings` clean.

**Task 7.0d AC:** `[T]` `helm template` renders the policy with a non-empty `from` selector under default **and** demo values · `[T]` an empty selector triggers `{{ fail }}` (mirroring `ui-networkpolicy.yaml`'s existing allow-all guard) · `[H]` `make demo-up` from a clean cluster still works with zero configuration (NFR26).

**Parallelizable:** all four fully concurrent; none depends on another.
**Milestone Value:** three schedule surprises and one unauthenticated-surface class are removed before any feature task starts.

### Milestone 7.1: Bounded investigator concurrency (Q9)

| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 7.1 | Pure admission core — `controllers/admission.rs`: `slot_is_occupied`, `severity_band`, `admission_decision`, `queue_mark_patch`/`clear_queue_marks_patch`. No K8s, no behavior change | M | 7.0a | pending |
| 7.2 | Wire the gate into the `Pending` reconcile arm + `InvestigatorConfig` env vars + the two `InvestigationStatus` fields. **Code default `0` = unlimited — no behavior change** | M | 7.1 | pending |
| 7.3 | Queue observability (`GET /api/v1/scheduler/stats` from in-process counters) **and, separately gated, chart enablement** (`maxConcurrentInvestigations` env wiring, chart default `2`, deployment-guide `5`→`2` correction) | M | 7.2 | pending |

**Task 7.1 AC:** `[T]` `admission_decision` is a strict total order — identical inputs always yield an identical permutation · `[T]` **impact banding:** an investigation with no `impact_score` never reaches band 0 even at `severity: Critical`; an SLO-registered Critical outranks an unregistered Critical · `[T]` **provenance reservation:** at `cap=4` with 2 slots held by unregistered services a third unregistered item defers while a registered one admits; at `cap=1` an unregistered item never takes the only slot while a registered one is queued · `[T]` **preemptive escalation:** with band 0 saturated, an item past `escalate_after` is admitted within `escalation_share_every` admissions · `[T]` `severity_band()` pinned for all four `Severity` values by an explicit test (**no derived `Ord`** — `Severity` derives none today, and deriving it would couple band order to variant declaration order) · `[T]` `slot_is_occupied` is `false` for `succeeded==1`, `false` for `failed>=1`, `true` for `active>=1` (proving `ttlSecondsAfterFinished` leftovers cannot wedge the queue) · `[T]` `clear_queue_marks_patch()` emits explicit JSON `null`s (merge-patch **omits** `None` fields, so "cleared on admission" is otherwise a silent no-op) · `[T]` `cargo fmt`/`clippy -D warnings`/`cargo test` green.

**Task 7.2 AC:** `[T]` `InvestigatorConfig::from_env` parses all five new vars with defaults `0`/`15`/`900`/`3`/`2`; existing env tests unchanged · `[T]` the two new `InvestigationStatus` fields round-trip and appear in the chart CRD (7.0a guard) · `[T]` `InvestigationPhase` gains **no** variant and no existing API response field changes shape — asserted by the full UI/BFF suites (2859 pytest / 767 vitest / 99 e2e) passing **unmodified** · `[O]` with `max_concurrent=0` on a live cluster the `Pending` arm creates the Job with **no Jobs-list call** · `[O]` with `max_concurrent=2` and 2 occupied slots a Pending investigation gets no Job, stays `Pending`, and is admitted within one poll interval when occupancy drops · `[O]` a Jobs-list API error results in admission with a warning logged (fail-open, per the RFC 0001 S5 precedent).
*Test-shape note:* reconcile-level behavior is `[O]`, not `[T]`, and deliberately so — there is **no kube-client test harness anywhere in `operator/`** (no `operator/tests/`, no reflector; `api.rs:2985` says so in the source) and AD-8 accepts manual integration verification. The pure core carries the `[T]`s, following the existing `service_guard_skip_reason` precedent. Building a mock `kube::Client` harness was considered and rejected as net-new shared infrastructure (RFC 0003 §7 A6).

**Task 7.3 AC:** `[T]` `GET /api/v1/scheduler/stats` returns the documented shape from in-process counters, issues **no** Jobs list per request, and reports `max_concurrent: 0` without erroring when disabled · `[T]` Helm render test asserts `BEEPER_INVESTIGATOR_MAX_CONCURRENT` appears in the rendered operator Deployment with the value from `llm.maxConcurrentInvestigations`, and that demo values render `2` · `[T]` `docs/deployment-guide.md` states `2`, matching both values files, guarded by a doc-vs-values consistency test · `[H]` an operator approves the **FR11-vs-Q9 default reconciliation** (code `0`, chart `2`) as the shipped posture — a documented contradiction that must not be settled silently in a commit · `[H]` **NFR26 program gate:** `make demo-up` from a clean cluster works with zero configuration under the new chart default · `[O]` during a burst, `kubectl get inv` shows exactly `cap` investigations with a Job and the rest `Pending` with `queued_at` set.
*Split note:* the observability half is inert; the chart flip is a live behavior change to **every** install including the demo. Land them as separable commits behind the one `[H]` so the increment keeps its default-off property.

**Parallelizable:** 7.1 → 7.2 → 7.3 strictly sequential (each builds on the last); concurrent with Milestone 7.2 and with react-ui Milestones 2.4/2.5.
**Milestone Value:** a wide outage produces investigations that *complete*, slowly, instead of a system that produces nothing.

### Milestone 7.2: Restart resilience & temporal anchoring (Q11)

| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 7.4 | **`triggered_at` from anomaly onset** — derive `spec.triggered_at` from `event.timestamp_ms`, not `Utc::now()` | S | 7.0c | pending |
| 7.5 | **Startup grace period** for detector firing (`BEEPER_DETECTION_STARTUP_GRACE_SECS`, default 600, on) | S | 7.0c | pending |
| 7.6 | **Burst soak + restart live validation** on kind | M | 7.3, 7.5 | pending |

**Task 7.4 AC:** `[T]` the pure event→`InvestigationSpec` mapping derives `triggered_at` from `event.timestamp_ms` · `[T]` the BFF detail payload and the React summary header render correctly when `triggered_at < started_at` — the **new normal** — with fixtures updated.
*Why its own task:* this is not a 2-line change. `triggered_at` flows through `api.rs` → `InvestigationService.get_investigation()` → `investigation_detail_json`'s `metadata.triggered_at` → the React summary header, so it changes what users see, changes pinned fixtures, and changes the meaning of an existing CRD field with objects already on cluster. It is owned here because Milestone 7.2 already touches `consumer.rs`, and it is a **hard contract** [RFC 0005](../specs/rfcs/0005-cross-service-incident-correlation.md)'s causal-ordering gate depends on. Whoever touches `consumer.rs` first owns it; the other must not regress it.

**Task 7.5 AC:** `[T]` within the grace window a detector that would otherwise emit an `AnomalySignal` emits none; after the window, identical input fires normally · `[T]` the grace decision is driven by detector-internal sample/stability state, **not** by `DetectionStats::ewma_warmup_samples` (known to read 0 while detectors fire — see the project memory note; the `[T]` must assert on detector state) · `[T]` `BEEPER_DETECTION_STARTUP_GRACE_SECS=0` restores exactly today's behavior · `[T]` if RFC 0002's dwell gate has already landed, a `[T]` proving the two gates compose (a dwell breach inside the grace window still suppresses; after grace, dwell is unchanged) · `[O]` an operator restart on the demo cluster produces ≤2× the steady-state investigation count in the following 15 minutes.
*Honest framing:* this is a **blind window**, not a free win — a restart during a real incident means the incident is not detected for up to 10 minutes. It trades a false-positive burst for a false-negative window, and is right only because the burst *also* disables the system while the window merely delays it.

**Task 7.6 AC:** `[O]` synthetic ≥20-service burst with `maxConcurrentInvestigations: 2`: ≥90 % reach a terminal phase, versus the recorded 0/23 and 0/48 · `[O]` minimum host free memory ≥1 GB throughout, versus the recorded 78–92 MB · `[O]` **no investigation is dropped** — CRDs created equals what the per-service guard admits, and the created-vs-completed difference is fully accounted for by the queue · `[O]` API-server QPS delta from admission polling measured and recorded (the Jobs list is a real API call — this codebase has **no reflector**, so occupancy is cached behind the admission mutex rather than listed per reconcile) · `[O]` **fairness under attack:** with the queue saturated by synthetic anomalies on unregistered service names, an injected fault on an SLO-registered service is admitted within one drain cycle · `[H]` an operator confirms the queue behaves comprehensibly — depth visible, oldest wait visible, a high-impact service admitted ahead of queued low-impact ones.
*Budget note:* each soak needs a fresh cluster plus the ~8–10 min warmup before faults can be injected, and each investigation is 12–15 min on local qwen. This task is mostly **waiting**.

**Parallelizable:** 7.4 and 7.5 concurrent after 7.0c; 7.6 after both 7.3 and 7.5.
**Milestone Value:** an operator restart becomes a non-event instead of a noise generator, and incident onset becomes a truthful timestamp.
**Observational Outcomes:** `[O]` M1 completion ratio; `[O]` M2 host memory; `[O]` M3 post-restart investigation count; `[O]` M4 fairness under a forged-anomaly burst (all defined in RFC 0003 §2).

---

## Phase 8: Cross-Service Incident Correlation (Expansion, 2026-08-11 — GATED)

RFC 0001 Phase 3. Design: [RFC 0005](../specs/rfcs/0005-cross-service-incident-correlation.md). Traces to **FR46**, **FR48**, **NFR26**, **NFR29**, **NFR30**, **NFR31**. Unblocks [docs/plans/react-ui.md](react-ui.md) Task 7.1.

> **This phase is gated on Task 8.1a returning a go.** If the dependency graph is too sparse to gate on, the correct outcome is to stop: FR48 stays suppressed, react-ui Task 7.1 closes as won't-do, and the RFC's value was the measurement it forced. That is a **result, not a failure** — record it as such. RFC 0001 §10's Q10 lesson is the precedent: a plausible hypothesis predicted 23→handful and measured 23→21.

### Milestone 8.1: Correlation

| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 8.1a | **Phase 0a — dependency-graph density measurement + go/no-go** | S | None | pending |
| 8.1b | **Phase 0b — candidate-pair labelling + committed fixture corpus** | S | 7.6, 8.1a (go) | pending |
| 8.2 | Dependency graph in the operator (`correlation/graph.rs`, RBAC, refresh loop, flag-gated debug endpoint) — nothing consumes it yet | L | 7.0a, 7.0d, 8.1a (go) | pending |
| 8.3 | Correlator + CRD fields + reconciler, behind `BEEPER_CORRELATION_ENABLED`, **default false** | L | 7.4, 8.1b, 8.2 | pending |
| 8.4 | Surface through operator REST + BFF + SSE + React `BlastRadiusPanel` — **this is react-ui Task 7.1** | M | 8.3, react-ui 10.1 (`DependencyGraph` primitive) | pending |
| 8.5 | Live validation on kind + enable-by-default decision | S | 8.4 | pending |

**Task 8.1a AC:** `[O]` measured edge count of the env-var-discovered graph across otel-demo, with a breakdown of which edges come from `*_ADDR`-style conventions versus anything else · `[H]` **go/no-go recorded in RFC 0005. No-go ⇒ stop.**
*Why split from 8.1b:* graph density depends on nothing and is the actual gate; pair labelling wants the calm baseline Task 7.6 produces. Splitting pulls the go/no-go weeks earlier at zero cost — and a no-go collapses an L+L+M+S phase to a paragraph.

**Task 8.1b AC:** `[T]` a **committed** fixture corpus exists — `kubectl get investigations -o json` snapshots across a warmup burst plus a labelled candidate-pair file — loadable by an offline replay harness · `[O]` human ground-truth labels on ≥20 candidate pairs yielding a precision figure (NFR29).
*Why it is a task, not an assumption:* there is no `operator/tests/` directory and no captured incident data in the repo — the 23/25 and ≈14 figures exist only as prose. Task 8.3's storm-suppression replay is not a legitimate `[T]` until this corpus is committed. Capture needs a live cluster plus the ~8–10 min warmup per run.

**Task 8.2 AC:** `[T]` **cross-language parity:** the Rust classifier and `service_topology.py` produce identical `upstream`/`downstream`/`blast_radius` from **one** shared committed input fixture against **one** shared committed expected-output file, loaded by both `cargo test` and `pytest` (two suites each asserting against their own copy cannot detect disagreement) · `[T]` pod env-var scanning matches hostnames only and **never logs, persists, or returns env values** — an explicit negative test, and the single most important test in this phase, because env values carry credentials and the operator ClusterRole already has cluster-wide `secrets` read · `[T]` a manifest test asserts the new `services`/`endpoints` verbs exist, namespaced where the deployment shape allows · `[T]` `GET /api/v1/topology/graph` is not registered at all when `BEEPER_TOPOLOGY_API_ENABLED` is unset (the operator API has **no authentication**; a full dependency graph is reconnaissance-grade) · `[O]` a graph-refresh failure degrades to last-known and never blocks or panics the detection loop · `[O]` refresh cost and API-server QPS delta measured.
*Sizing note:* re-estimated from M to **L (≈7–10 days)**. The cross-language parity harness alone needs a committed serialization format, assertions in two suites, and a lift of `_classify_topology`/`_bfs` to module-level pure functions in Python.

**Task 8.3 AC:** `[T]` links B→A on a 1-hop ordered pair; **refuses when the graph direction is opposite**; refuses when onset lag exceeds the window; refuses when no path exists at any hop count · `[T]` **replayed against the committed 23/25 warmup-burst fixture, the storm gate trips: `wide_outage`, zero causal directions asserted** · `[T]` the pure `status_diff` returns `None` on unchanged input (idempotence) and emits a retraction when the upstream goes terminal past `overlap_grace` · `[T]` the serialized status patch body preserves `correlations` across an investigation-controller phase transition (merge-patch omits `None`, which is what makes this safe — and load-bearing, so it is pinned) · `[T]` an Investigation with no correlation fields round-trips unchanged · `[T]` the 7.0a CRD-parity guard passes with the four new fields in **both** the Rust struct and the ~60 lines of hand-written chart YAML · `[T]` with the flag off, zero patches and `helm template` matches a **committed golden manifest** under default and demo values (NFR26).

**Task 8.4 AC:** `[T]` operator REST detail includes the four fields, omitted when empty; the list response gains only `correlation_group` + `correlated_service_count` (FR46's no-horizontal-scroll bar) · `[T]` BFF contract test: `metadata.correlated_services` populated end-to-end from a fixture CR · `[T]` `BlastRadiusPanel` **composes** the shared `DependencyGraph` primitive from react-ui Task 10.1 and renders its sr-only tabular fallback — it does not reimplement it · `[T]` React renders upstream/downstream with hedged copy and a visible basis; **absent correlation still renders nothing** (D1 preserved — `InvestigationDetailPage.test.tsx:283` must pass unmodified) · `[T]` an SSE `correlation` event updates an already-open detail page (note: `_generate_json_sse_events` does not diff arbitrary metadata fields today — this diffing is net-new and budgeted here) · `[T]` `blast_radius` **counts** render for a `user` session while the downstream **service-name list** requires `admin`, structurally and behaviorally (the counts are derived from the K8s graph, so they disclose topology facts about services with no incident at all) · `[T]` Storybook story present; `lint:terms` clean · `[H]` a reviewer shown a correlated pair cold names the upstream service in ≤5 s · `[H]` copy review confirming no sub-threshold link reads as proven causality.

**Task 8.5 AC:** `[O]` the RFC 0001 §7 DB-pool→downstream scenario produces exactly two **linked** incidents with correct direction, not two orphans · `[O]` false-link rate over a full warmup cycle meets NFR29 · `[O]` zero causal directions asserted during a >50 %-affected event · `[H]` default-on decision recorded · `[T]` `make demo-up` unaffected under either default.

**Parallelizable:** 8.1a immediately and independently; 8.2 after the go; 8.1b concurrent with 8.2 once 7.6 lands; 8.3 after 8.1b+8.2+7.4; 8.4 after 8.3 and react-ui 10.1; 8.5 last. Up to 2 concurrent.
**Milestone Value:** opening any member incident of a cross-service event names the probable origin, the actually-affected downstream set, and the confidence — or, where the evidence is weak, says nothing at all.
**Observational Outcomes:** `[O]` faster scoping of multi-service incidents (react-ui Milestone 3.1's stated outcome); `[O]` link precision ≥0.9 on labelled live pairs.
