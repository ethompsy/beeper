...
## Identity & Access (Expansion — ADR 0002)

**Status:** Proposed — resolves the React workstream's Q11 ("where does verified user identity come from?"); architecture recorded in [ADR 0002](../specs/decisions/0002-oidc-scim-and-local-fallback-identity.md) (Status: Proposed, awaiting `[H]` approval). Continues the FR/NFR numbering; implemented as Milestone 2.3 of [docs/plans/react-ui.md](../plans/react-ui.md).

### Functional Requirements (Identity & Access)

- FR54: The UI supports exactly three authentication modes via a single configuration knob (`BEEPER_AUTH_MODE=none|local|oidc`, default `none`). `none` preserves today's behavior exactly (demo/dev: zero configuration, `X-Beeper-Role` dev affordance per `ALLOW_ROLE_HEADER`; production: fail-closed, no admin path). Contradictory configuration (e.g. SCIM enabled outside `oidc` mode, `local`/`oidc` without an env-supplied `SECRET_KEY`) is refused at application startup with a clear error. In no mode does role resolution honor an unverified bearer token — the unverified JWT `groups`-claim path is deleted in all modes, guarded by a permanent regression test
- FR55: In `oidc` mode, users authenticate via an OIDC authorization-code flow performed server-side by the Flask BFF, with full ID-token validation (signature via issuer JWKS restricted to asymmetric algorithms, exact `iss`, `aud`, `exp`, `nonce`, and `state`); the resulting credential is an `HttpOnly`, `SameSite=Lax` session cookie carrying an identity snapshot, never tokens (tokens are validated at the callback and discarded). Unauthenticated `/api/v1/*` and SSE requests receive RFC7807 `401 authentication-required` (distinct from the existing `403 permission-denied`); unauthenticated page requests round-trip through login and return to the exact original permalink (preserving FR53 access-consistency)
- FR56: The two-tier role (`admin`/`user`, unchanged — no new tiers) is derived from IdP group membership via one configurable group set shared by login and provisioning: `BEEPER_ADMIN_GROUPS` (case-insensitive, default `Admins,beeper-admin`) and optional `BEEPER_USER_GROUPS` (empty = any authenticated principal is `user`). When SCIM provisioning is enabled, the provisioned store is authoritative at request time (store-primary, ≤60 s propagation per NFR25) and login-time claims are never authoritative; when SCIM is disabled, the login-time claims snapshot governs for the bounded session lifetime, and this limitation is documented
- FR57: The UI exposes a SCIM 2.0 provisioning API (`/scim/v2`: Users, Groups, discovery documents) sufficient for Okta, Entra ID, and Keycloak provisioning clients: create/read/filter (`eq` on `userName`/`externalId`/`displayName`), update via PUT and PATCH (including both vendor membership-delta dialects), deactivation (`active=false`), and hard DELETE; errors use the SCIM error schema (a recorded deviation from the RFC7807 house style, scoped to `/scim/v2/*`). All IdP-pushed groups are stored and inspectable (passthrough); role assignment derives solely from the FR56 group set
- FR58: SCIM endpoints authenticate with a long-lived bearer token supplied via a Kubernetes Secret (constant-time comparison), support dual-token zero-downtime rotation, fail closed when enabled but unconfigured, and are not registered at all when disabled; every provisioning mutation is audit-logged with a token fingerprint (never the token), with admin-group membership changes flagged distinctly
- FR59: In `local` mode (SSO disabled), users authenticate with admin-managed local username/password accounts (argon2id at rest); login is required for all UI/API access; there is no self-registration, no email flow, no MFA, and no password self-service — admin reset covers recovery. Deactivated users are denied within one cache interval; login failure responses do not distinguish unknown-user, bad-password, and deactivated
- FR60: An admin-gated React view (`/app/admin/users`, Manage group) lists users (role, origin, active status, last login) and lets admins create local users, assign the Admins/Users role, reset passwords, and deactivate/reactivate accounts. The last active admin can be neither demoted nor deactivated via the admin API (`409 last-admin`); SCIM-linked records are read-only while in `oidc` mode (`409 scim-owned-user`); any write path that leaves zero active admins raises a CRITICAL log and a health-endpoint flag
- FR61: The first `local`-mode admin is seeded non-interactively from a Kubernetes Secret (Helm-supplied env) with an idempotent never-overwrite rule (a rotated password is never reverted by a restart), with a CLI (`flask create-admin`) as the recovery path; `none` mode (the demo) requires zero bootstrap configuration
- FR62: One shared identity store (Qdrant, payload-only collections) serves both writers — SCIM in `oidc` mode, the admin UI in `local` mode — keyed canonically by casefolded username/email with `externalId` as the machine join key. A SCIM create matching an existing local record adopts and links it, recomputing the role authoritatively from IdP group membership (never inheriting the prior local role); local records and their password hashes persist across mode switches, but local password login is functional only in `local` mode

### Non-Functional Requirements (Identity & Access)

- **NFR25 — Provisioning propagation:** a provisioning change (deactivation, role demotion/promotion) takes effect on live sessions within 60 s of the SCIM request arriving at the UI, without re-login — including termination of in-flight SSE streams whose identity loses the required role
- **NFR26 — Demo preservation:** with `BEEPER_AUTH_MODE=none` (the default), `make demo-up`/`make demo-deploy` require zero identity configuration and zero interactive steps; the demo-values Helm manifest is byte-identical to the pre-ADR-0002 output except the explicit `FLASK_ENV` value; and the full pre-existing UI test suite passes unmodified

## Reliability, Evidence & Correlation (Expansion — RFCs 0003–0005)

**Status:** Proposed — designs recorded in [RFC 0003](../specs/rfcs/0003-bounded-investigator-concurrency.md) (bounded investigator concurrency + restart-resilient detection, closing `main.md` Q9/Q11), [RFC 0004](../specs/rfcs/0004-investigation-evidence-exposure.md) (evidence persistence contract + exposure boundary, closing `react-ui.md` Q8), and [RFC 0005](../specs/rfcs/0005-cross-service-incident-correlation.md) (cross-service correlation, RFC 0001 Phase 3, gated on a go/no-go measurement). Continues the FR/NFR numbering. Implemented as Phase 7/8 of [docs/plans/main.md](../plans/main.md) and Milestones 2.4/2.5 of [docs/plans/react-ui.md](../plans/react-ui.md).

### Functional Requirements (Reliability, Evidence & Correlation)

- FR63: Investigations deferred by the global concurrency bound (FR11) are admitted in a deterministic, starvation-free order that is not controllable by telemetry submitted to the unauthenticated ingestion endpoint: the primary band is the operator-configured customer-impact score (a service with no SLO entry can never reach the top band), a bounded share of concurrent slots and queue positions is reserved against services absent from the operator-configured service set, and age-based escalation is preemptive — an investigation queued beyond the escalation window receives a guaranteed share of admissions regardless of band. Queue depth, per-investigation queue age, the effective cap, and the count of reserved slots in use are observable through the operator API without issuing a live Kubernetes list per request
- FR64: An operator restart does not produce a detection false-positive burst: detector firing is suppressed until baselines are stable (a startup grace period, default on), and the suppression decision derives from detector-internal sample and variance state rather than from the reported warmup statistic. Optional persistence and restoration of detector baselines is designed but not enabled; if introduced it must treat the persisted snapshot as untrusted input, bound the authority restored from it, and fail closed to a cold start on any validation failure
- FR65: Evidence exposed through the JSON API is governed at the serialization boundary by content sensitivity rather than by role tier alone: query text and numeric metric values are available to any authenticated principal, while raw log excerpts require both the `admin` role and an explicit deployment opt-in (default off, including in the zero-configuration demo). Every served item passes a redactor covering credential, token, and personal-data shapes plus a shape-agnostic high-entropy backstop; a redaction failure suppresses the item rather than emitting it; and evidence renders as text only — never as markup

### Non-Functional Requirements (Reliability, Evidence & Correlation)

- **NFR27 — Concurrency-bound integrity:** with a single operator replica, live investigator Jobs never exceed the configured maximum; a freed slot is filled within one queue-poll interval (default 15 s); no anomaly admitted by the per-service guard is ever dropped by the concurrency gate; and slot occupancy is derived from live Job state so that finished-Job retention (`ttlSecondsAfterFinished`) can never wedge the queue. Above one replica the bound degrades to per-replica — a documented, silent-overshoot limitation that must be recorded alongside the other single-replica invariants
- **NFR28 — Evidence off the critical paint path:** evidence and related-knowledge data are never fetched on the request that gates first paint (NFR19) nor inside the detail event stream's poll loop (NFR21); the evidence block renders within 1 s of first paint at p95; and an unavailable evidence store degrades the evidence block only, never the detail view, returning an explicit "not available" result rather than an error
- **NFR29 — Correlation precision:** a written causal link is correct ≥90 % of the time when measured against human-labelled candidate pairs from live runs (recall is explicitly secondary); causal direction is derived solely from the discovered dependency graph and never inferred from event timing; and during a wide event — more than half of known services incident-open, or a candidate group above the configured size bound — no causal direction is asserted at all
- **NFR30 — API authorization completeness:** every route on the BFF's `/api/v1/*` surface carries an explicit declared role, machine-verified against a committed allowlist by enumerating the application's URL map — so a route added without a role declaration fails the build rather than shipping as an unauthenticated surface. Role declarations are asserted on two axes, structurally (the declared role) and behaviorally (an under-privileged session receives 403); client-side hiding of controls is never treated as enforcement
- **NFR31 — Operator API exposure containment:** the operator's HTTP surfaces (the API on :8080 and telemetry ingestion on :9090) are reachable only from the workloads that legitimately call them, enforced by a shipped NetworkPolicy that fails closed on an empty selector rather than defaulting to allow-all. Any new operator API route returning cluster topology or dependency structure is disabled by default and may be enabled only where that containment is in place
