...
### Q12/Q13 — landed, and the residual (→ RFC 0002)
**Q12 implemented and live-measured (2026-06-04): 21 → 11**, with **0** remaining `*_bucket` investigations (PR #20). **Q13a/Q13b** (PR #21, stacked) extend the denylist to observability-infra/static metrics (`otelcol_`, `jvm_memory_limit/init`) — **GC and dynamic heap occupancy are deliberately kept** (heavy GC is a real symptom; Q12 already surfaces it as a rate) — and add an *opt-in* absolute stddev floor. The last residual (idle counter-rate streams) is **not** fixable by any single global floor — count-rate noise and small-but-real GC-duration signal overlap in absolute magnitude across different units — so it is split out as **Q13c** with its own design: **RFC 0002 — Detection significance gating** (unit-free temporal-persistence dwell + optional unit-aware magnitude gate, gated on a Phase-0 live characterization). Progression so far: **38k → 23 → 21 → 11** (~99.97 %).
